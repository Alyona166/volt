<?php
// Полная диагностика и тестирование проекта VOLT
...
if ($testUserId && $testServiceId) {
    try {
        ...
        $stmt->execute([$testUserId, $testServiceId, 'Тестовое устройство', 'Тестовая заявка', 'Новый']);
        $testOrderOk = true;
...
// 9. Проверка структуры users
try {
    $stmt = $pdo->query('DESCRIBE users');
    $fields = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $results[] = resultRow('Поля таблицы users', true, implode(', ', $fields));
} catch (Exception $e) {
    $results[] = resultRow('Поля таблицы users', false, $e->getMessage());
}

// 10. Проверка структуры services
try {
    $stmt = $pdo->query('DESCRIBE services');
    $fields = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $results[] = resultRow('Поля таблицы services', true, implode(', ', $fields));
} catch (Exception $e) {
    $results[] = resultRow('Поля таблицы services', false, $e->getMessage());
}

// 11. Проверка структуры feedback
try {
    $stmt = $pdo->query('DESCRIBE feedback');
    $fields = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $results[] = resultRow('Поля таблицы feedback', true, implode(', ', $fields));
} catch (Exception $e) {
    $results[] = resultRow('Поля таблицы feedback', false, $e->getMessage());
}
...
<head>
    <meta charset="UTF-8">
    <title>Полная диагностика и тест VOLT</title>
    <style>
        body { font-family: Arial, sans-serif; background: #222; color: #eee; }
        h1 { color: #27ae60; }
        table { border-collapse: collapse; width: 100%; max-width: 1000px; margin: 2rem auto; background: #333; }
...
<body>
    <h1>Полная диагностика и тестирование проекта VOLT</h1>
    <table>
    ...
    <div class="hint">
        ...
        <p>Проверьте, что вы авторизованы при тестах из личного кабинета!</p>
    </div>
</body>
</html> 